...
##############################
# NECESSARIO CHECAR A VIABILIDADE
########################
    ...
    def updateRodadaMaxMinAutomatic(self):
        ...
        for row in rows:            
            id=row[0]        
            sql="SELECT max( total_votacao ) , max( total_presenca ) ,  max( total_proposicao ) , max( total_despesa ),rodada FROM `pontuacao` where rodada = "+str(id)+" group by rodada"
            print sql
            novos = db.session.execute(sql)
            #novos = db.session.fetchall()
            for novo in novos:  
                print novo[0]              
                sql="update rodada set max_votacao="+ str(novo[0])+ ", max_presenca="+ str(novo[1])+ ",max_proposicao="+ str(novo[2])+ ",max_despesa="+ str(novo[3])+ " where id="+ str(novo[4])
                print sql
                db.session.execute(sql)

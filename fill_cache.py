...
PEC_dir = os.path.abspath(os.path.join(base_dir, 'scripts/growth_lib/'))
sys.path.append(PEC_dir)
from pecbrasil import app
from flask import g

''' Connect to DB '''
db = MySQLdb.connect(host="localhost", user=environ["PEC_DB_USER"], 
                        passwd=environ["PEC_DB_PW"], 
                        db=environ["PEC_DB_NAME"])
db.autocommit(1)
cursor = db.cursor()
